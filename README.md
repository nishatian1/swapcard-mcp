...
| 5a | Dockerize + Caddy deploy on `mcp.fhsagents.site/swapcard` | ✅ **live** |
| 4 | Web "paste your key" OAuth wrapper (claude.ai browser) | ✅ **live** (web + desktop) |
...
Settings → **Connectors** → **Add custom connector** → URL `https://mcp.fhsagents.site/swapcard`.
Claude runs an OAuth flow that shows a **"Paste your Swapcard API key"** page; paste the key once and
you're connected. (The key is validated against Swapcard, then carried in an encrypted token — the
server stores nothing.)

### Claude Desktop / Code via config (no OAuth)
Alternatively, connect via the `mcp-remote` bridge with your key as a header:
